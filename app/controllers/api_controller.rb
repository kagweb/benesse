...
class ApiController < ApplicationController
  ...
  def projects
    ...
    @test_projects = Project.where(:test_upload_at.gte => target_day, :test_upload_at.lt => DateTime.now).where(status: 3).where(registration_status: true)
    @production_projects = Project.where(:production_upload_at.gte => target_day, :production_upload_at.lt => DateTime.now).where(status: 5).where(registration_status: true)
  ...
  def file_structure
    render file: 'api/file_structure'
  end
