...
class ApiController < ApplicationController
  ...
  def projects
    ...
    @test_projects = Project.where(:test_upload_at.gte => target_day, :test_upload_at.lt => target_day + 1.day).where(status: 3).where(registration_status: true, upload_server: 'NKD')
    @production_projects = Project.where(:production_upload_at.gte => target_day, :production_upload_at.lt => target_day + 1.day).where(status: 5).where(registration_status: true, upload_server: 'NKD')
